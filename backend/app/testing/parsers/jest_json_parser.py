"""
Jest JSON result parser — extracts structured test results from the
Jest JSON reporter output (--json).
...
class JestJsonParser(TestResultParser):
    """Parse Jest JSON reporter output into structured TestFailure records."""
    ...
    def can_parse(self, process_result: ProcessExecutionResult) -> bool:
        """Detect Jest JSON output.

        Jest suites always carry a ``perfStats`` key (Vitest never does),
        which is the primary discriminator from Vitest's reporter.
        """
        ...
        if not isinstance(suites, list) or not suites:
            return False
        return all(isinstance(s, dict) for s in suites) and any(
    ...
    def parse(
        ...
        """Parse Jest JSON into structured results."""
