"""
PHP symbol parser — extracts symbols and relationships using tree-sitter.
...
def _get_parser() -> Optional[Any]:
    ...
    try:
        ...
        import tree_sitter_php as tsphp  # type: ignore[import-untyped]
...
class PhpSymbolParser:
    """Parse PHP source and extract structured symbols with relationships."""
    ...
    def parse(self) -> Tuple[List[GraphNode], List[dict], List[str]]:
        ...
        if parser is None:
            self.diagnostics.append(
                "PHP parser unavailable: tree-sitter-php package not installed. "
    ...
    def _extract_namespace(self, node: Node, parent_id: str) -> None:
        for child in node.children:
            if child.type in ("name", "namespace_name"):
                ns = self._node_text(child)
                symbol_id = make_symbol_id(self.file_path, f"namespace:{ns}")
                sl, _ = self._node_lines(node)
                self.symbols.append(GraphNode(
                    id=symbol_id, name=ns.split("\\")[-1], qualified_name=f"namespace:{ns}",
                    kind="namespace", file_path=self.file_path, language=self.language,
                    start_line=sl, end_line=sl, metadata={"namespace": ns},
                ))
                return
    ...
    def _extract_class(self, node: Node, parent_id: str) -> None:
        ...
        bases = []
        for child in node.children:
            if child.type in ("base_clause", "superclass"):
                for sub in child.children:
                    if sub.type in ("name", "namespace_name"):
                        bases.append(self._node_text(sub))

        is_abstract = "abstract" in self._node_text(node)
        is_final = "final" in self._node_text(node)
        ...
        self.symbols.append(GraphNode(
            ...
            metadata={"bases": bases, "is_abstract": is_abstract, "is_final": is_final},
        ))
        self.relationships.append({
            "source_id": parent_id, "target_id": symbol_id,
    ...
    def _extract_trait(self, node: Node, parent_id: str) -> None:
        ...
        self.symbols.append(GraphNode(
            id=symbol_id, name=name, qualified_name=qname,
            kind="trait", file_path=self.file_path, language=self.language,
            start_line=start_line, end_line=end_line, parent_id=parent_id,
        ))
        self.relationships.append({
            "source_id": parent_id, "target_id": symbol_id,
    ...
    def _extract_method(self, node: Node, class_id: str, class_name: str) -> None:
        ...
        for child in node.children:
            if child.type == "visibility_modifier":
                visibility = self._node_text(child)
            ...
            if child.type == "property_element":
                for sub in child.children:
                    if sub.type == "variable_name":
                        prop_name = self._node_text(sub).lstrip("$")
                        p_id, p_qname = self._make_symbol(prop_name, [class_name])
                        pl, _ = self._node_lines(sub)
                        self.symbols.append(GraphNode(
