...
> **Last updated**: August 14, 2026 (session 47 — **Phase 21 Run Replay & Deterministic Reproduction + Enterprise Audit Dashboard COMPLETE**)
> **Phase 21 (Session 47)**: the replay subsystem from the previous session is now a first-class dashboard experience on the run-detail page (`/dashboard/runs/[id]`) — **REPLAY & AUDIT** section consuming only real APIs: manifest status / repository fingerprint / stages recorded / last replay; verdict banner (MATCH/DRIFT/INVALID/INCOMPLETE, color-coded); **Start replay** EXACT / DETERMINISTIC / COMPARE (compare-run picker from the newest 50 runs) with an idle→starting→running→completed/failed phase machine + clean error/retry; **Replay Timeline** (per-stage comparison status + deterministic/LLM-proposed/observational classification + recorded-vs-replay fingerprints); **Difference Viewer** (bounded ≤50, categorized: repository/config/artifact/stage-input/stage-output/test-result/review/quality-gate/decision drift + missing evidence, severity + deterministic evidence — no claimed causality beyond recorded checks); **Audit Report** (enterprise summary banner — AUDIT RESULT / DRIFT DETECTED, primary difference + supporting evidence, identity, stage summary, expandable checks); **Replay History** (mode/verdict/checks/timestamp, paginated). Live updates reuse the existing run WebSocket: terminal-status push refreshes manifest+history (no new WS system); the existing polling fallback covers disconnected mode. Frontend: `frontend/src/components/replay/{ReplaySection,ReplayTimeline,DifferenceViewer,AuditReport,ReplayHistory}.tsx`, pure logic `frontend/src/lib/replay/replayModel.ts` (verdict tones, difference categorization, stage views, run state machine), `replayApi` in `client.ts`. CLI improvement: `replay`/`replay-compare` now return CI exit codes (0 match, 1 drift/incomplete, 2 invalid). Backend untouched otherwise (all replay endpoints already existed and are bounded). Tests: **frontend vitest 93 passed (9 files, +26)** — 20 `replayModel` unit tests + 6 `replayApi` contract tests; **backend 1889 passed / 17 skipped / 1 pre-existing env failure** (34 replay tests incl. 3 new CLI exit-code tests); `next build` EXIT=0; **live-PG UI-facing API E2E PASS** (manifest → EXACT MATCH → DETERMINISTIC MATCH → tampered workspace DRIFT → audit 8 checks → 4 persisted replay history entries → invalid-mode error → COMPARE 11 comparisons). Docs: `docs/RUN_AUDIT_AND_REPLAY.md`, report: `workflow-status/REPLAY_DASHBOARD_COMPLETION_REPORT.md`.
> **Session 46 (Aug 7)**: live model benchmark + LIVE E2E GREEN — benchmark-recommended capability chains wired into the live `.env` and `scripts/demo_phase17.py --live` ran end-to-end (demos A–E ALL PASS, autonomy goal completed, live `execute_run` terminal verdict **APPROVED** — first fully green approve run). Also: FixAgent JSON-repair fallback, `fix_array_of_lines_content`, doubled-brace + triple-quoted-string repair, and a real Session-34 validator fix (JSON-dict fallback values no longer mangled).
...
> **Production Reliability & Operations (Phase 20B, Session 45)**: router hardening in `app/llm/router.py` — health-based selection (recovering-probe > healthy > warming > unknown > degraded > unhealthy, min-sample guard `DEVPILOT_PROVIDER_HEALTH_MIN_SAMPLES`), recovery detection (bad-spell success ⇒ `recoveries` + warm-up), passive health probing (`app/services/provider_probe.py` background loop, out of the traffic window), post-failure cooldown (`DEVPILOT_PROVIDER_COOLDOWN_AFTER_FAILURE_SECONDS`), adaptive timeouts from measured latency; observability — `app/services/system_metrics.py` (runs/repositories/autonomy/providers/resources), `app/services/subsystem_status.py` readiness matrix, correlation IDs + request-size limit (`app/core/context.py`/`middleware.py`/`logging.py`), `app/services/provider_metrics_persistence.py` PG loop; startup validation `app/core/startup_validation.py` (fail-fast optional); endpoints `/health/live`, `/health/ready`, `/api/v1/operations/{status,metrics,startup-validation}`, `WS /api/v1/ws/system`; CLI `operations-status`/`operations-metrics`/`validate-config`; dashboard `/dashboard/operations`; `scripts/demo_phase20b.py` demos A–F ALL PASS. Full design: `docs/PRODUCTION_RELIABILITY.md`, report: `workflow-status/PHASE20B_COMPLETION_REPORT.md`
...
- **Repair:** 9/11 pass. nvidia 8b/49b failed because `FixAgent._extract_json`
  ...
  through `repair_json_text`; (b) `json_repair.py` gained two base passes —
  `fix_triple_quoted_strings` (nvidia-8b failure: `new_content` as Python
  triple-quoted blocks) and `fix_array_of_lines_content` (nemotron-49b
  failure: `new_content` as a JSON array of string lines with `#` comment
  lines — only fires when comments are present, so legitimate string arrays
  are never converted). Live repair probe: **`meta/llama-3.1-8b-instruct`
  0% → 100%** and **`nemotron-49b` 0% → 100%** (30s). Repair is green on every
  AVAILABLE model (only the 404 `deepseek-r1` and unpaid `deepseek-v4-flash`
  fail). 12 new tests (8 json_repair incl. triple-quote + array-of-lines
  heuristics; 4 FixAgent execute-level regressions pinned to real nvidia
  response shapes).
...
Phase 21 (Run Replay & Deterministic Reproduction + Enterprise Audit Dashboard) COMPLETE — Session 47: backend replay subsystem (models/service/API/CLI/migration 015/orchestration capture hook) + dashboard experience (ReplaySection/Timeline/DifferenceViewer/AuditReport/ReplayHistory on the run-detail page) + CLI CI exit codes. Verified: backend 1889 passed / 17 skipped / 1 pre-existing env quirk, frontend vitest 93 passed, `next build` EXIT=0, live-PG UI-facing API E2E PASS. See `docs/RUN_AUDIT_AND_REPLAY.md` + `workflow-status/REPLAY_DASHBOARD_COMPLETION_REPORT.md`.

**HTTP API path verified on the new chains (same day)** —
`scripts/verify_api_durability.py --live` PASSED: both HTTP paths
(`POST /api/v1/runs` → real `execute_run`, all 11 stages flowed, terminal
verdict `rejected`; autonomy goal API → `waiting_for_human`, 1 run persisted
`RUN-0A68CF4D`) persist runs/handoffs/consensus via PostgresRunStore ✅; CI
gates passed (rejected is a terminal verdict — the quality gate correctly
rejected the coding patch; a green *approve* through the HTTP API remains the
final polish).

**Next:** 1) extend `fix_triple_quoted_strings` for array-of-lines
`new_content` (nemotron-49b's residual failure) or chase the API-path approve
verdict (stronger coding model / richer fixture task); 2) enterprise roadmap
workstream E1.
