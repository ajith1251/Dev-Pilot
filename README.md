...
_Phase 20B (Production Reliability & Operational Hardening — the final production-hardening phase before closing Phase 20) strengthens DevPilot for long-running enterprise deployments: provider warm-up checks + automatic health probing + configurable post-failure cooldown + recovery detection + request-timeout optimization + health-based provider selection (all in `ProviderRouter`); operational resilience for transient network failures, rate limits, cold starts, PG reconnects and WebSocket reconnects; a resource-management audit (async task cleanup, DB session lifecycle, WS lifecycle, workspace/temp-file cleanup, bounded memory); expanded observability (run throughput, provider latency, failover/retry/recovery counts, repository processing time, autonomy durations, memory, active WS — `SystemMetricsService` + correlation-ID structured logging); startup configuration validation (`python -m app.cli validate-config`, `DEVPILOT_STARTUP_VALIDATION_STRICT` fail-fast); security hardening (request-size limits, redaction); enhanced health endpoints (`/health/live` liveness + `/health/ready` readiness with a subsystem matrix); and a new Operations Dashboard (`/dashboard/operations`) showing live system health, provider status, failover history, active runs, queue status, PostgreSQL/WebSocket/resource state — no mock data. Deterministic demo: `python scripts/demo_phase20b.py` (demonstrations A–F: outage→recovery, DB reconnect, leak-free long run, live ops dashboard, accurate health endpoints, graceful shutdown + restart recovery). See `docs/PRODUCTION_RELIABILITY.md`._
...
_Building on Phase 16 (autonomous goals, deterministic decisions, versioned replanning, budget enforcement, escalation, safe termination) and Phase 15 (handoffs, decisions, conflicts, memory promotion)._ _Phase 19B (Multi-Provider Failover & Reliability Platform) added the health-aware, failover-capable provider router (see `docs/MULTI_PROVIDER_ROUTING.md`)._ _Phase 20 workstream E adds dedicated test-result parsers for **unittest** (JUnit-style XML from `xmlrunner`/`unittest-xml-reporting`), **Vitest** (JSON) and **Jest** (JSON) alongside the pytest parser — structured counts, per-test failures with file/line/message, and deterministic classification instead of the generic fallback (Vitest/Jest discriminated by the `perfStats` key; see `docs/TESTING_AND_EXECUTION.md`). 12 new tests in `tests/test_testing.py`._ _Tests: **1696 passed / 18 skipped** on the live-PG full suite (the only failures are the pre-existing live-Gemini durability tests that need a fresh quota key); **zero regressions** in every deterministic suite — 60 `test_provider_router.py` tests green (43 Phase 19B + 11 Phase 20B capability fallbacks + 5 Phase 20B stream resume + 1 config_snapshot tier), 12 paid-tier Gemini tests in `test_llm_providers.py`, migration chain now 001→014. Raw-HTTP durability remains repeatable pytest (deterministic class in CI, live class gated on provider; item-13 bounded goal-path retry on ANY coding failure + task-analysis stage retry for the run-API path, see Sessions 17–19). The full deterministic live-PG suite (`-m "not live"`) is **1696 passed / 18 skipped / 1 failed**, the 1 failure being the pre-existing `test_wrapper_skips_cleanly_without_provider` env quirk._
...
| `test_provider_router.py` | **75** | **19B + 20B** | Router: classification, retry, circuit breaker, health, failover, streaming, snapshots, redaction, API, metrics store, typed per-capability fallbacks, mid-stream token-loss resume, Phase 20B reliability (probes, recovery, cooldown, selection) |
| `test_phase20b_provider_reliability.py` | **19** | **20B** | Probing, recovery detection, health-based selection (min-sample guard + probe priority), adaptive timeout, cooldown, probe loop |
| `test_phase20b_operations.py` | **17** | **20B** | Operations API, health endpoints, readiness, redaction, metrics |
| `test_startup_validation.py` | **14** | **20B** | Startup config validation + strict mode |
| **Total** | **1769+** | **All** | (17 skipped = live-GitHub/PG-dependent) |
